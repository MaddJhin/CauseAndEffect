...
public class ScrollRectSnap : MonoBehaviour
{
    ...
    public GameObject[] children;              // Child elements of the content panel to snap to
    ...
    void Start()
    {
        ...
        int bttnLenght = children.Length;
        distance = new float[bttnLenght];

        // Get Distance between buttons (second button position minus first button position)
        bttnsDistance = (int)Mathf.Abs(children[1].GetComponent<RectTransform>().anchoredPosition.x
                                       - children[0].GetComponent<RectTransform>().anchoredPosition.x);
    }
    ...
    void Update()
    {
        ...
        {
            distance[i] = Mathf.Abs(center.transform.position.x - children[i].transform.position.x);
        }
